...
class M_LoraConfig(PeftConfig):
    ...
    def __post_init__(self):
        self.peft_type = PeftType.M_LORA


class M_LoraModel(torch.nn.Module):
    """
    ...
    **Attributes**:
    ...
    def __init__(self, config, model):
        ...
        print('__init__ self.peft_config:', self.peft_config)
        self.model = model
        self._find_and_replace()
        print('self.peft_config.bias:', self.peft_config.bias)
    ...
    def _find_and_replace(self):
        ...
        print('merge_weights:', (self.peft_config.merge_weights or self.peft_config.inference_mode))
        ...
        for key in key_list:
            ...
            if target_module_found:
                ...
                if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
                    ...
                    # print('choice 2')
                    new_module = Linear(target.in_features, target.out_features, bias=bias, layer=int(key.split('.')[2] if not 'OLM' in self.peft_config.base_model_name_or_path else key.split('.')[-2]), **kwargs)
...
class Linear(nn.Linear, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        LoraLayer.__init__(self, r=r, lora_alpha=lora_alpha, lora_dropout=lora_dropout, merge_weights=merge_weights, number_of_adapter_pre_layer = number_of_adapter_pre_layer, layer = layer)
    ...
    def forward(self, x: torch.Tensor,         
        ...
        if self.disable_adapters:
            ...
            if self.r > 0:
                if weighted_matrix is None:
                    if not layer_wise_forward:
                        # print('sum over all A-B pair')
                        for i in range(self.number_of_adapter_pre_layer):
                            result += self.lora_B[i](self.lora_A[i](self.lora_dropout(x.to(self.lora_A[i].weight.dtype)))) * self.lora_scaling[i]
                    elif (layer_wise_forward and active_layer != self.layer):
                        for i in range(self.number_of_adapter_pre_layer):
                            result += self.lora_B[i](self.lora_A[i](self.lora_dropout(x.to(self.lora_A[i].weight.dtype)))) * self.scaling
                    else:
                        # print('you are not okay')
                        result += self.lora_B[active_adapter](self.lora_A[active_adapter](self.lora_dropout(x.to(self.lora_A[active_adapter].weight.dtype)))) * self.lora_scaling[active_adapter]
                else:
                    # print('doing weighting')
                    for i in range(self.number_of_adapter_pre_layer):
                        result += weighted_matrix[self.layer][i] * (self.lora_B[i](self.lora_A[i](self.lora_dropout(x.to(self.lora_A[i].weight.dtype))))) * self.number_of_adapter_pre_layer
        else:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
        ...
        # print('self.lora_scaling for layer', self.layer, ' ', self.lora_scaling)
