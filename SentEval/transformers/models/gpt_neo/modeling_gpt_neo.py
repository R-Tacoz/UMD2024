...
class GPTNeoSelfAttention(nn.Module):
    ...
    def forward(
        ...
        try:
            value, transport_plan = self.v_proj(hidden_states, layer_wise_forward = layer_wise_forward, active_layer = active_layer, active_adapter = active_adapter, weighted_matrix = weighted_matrix, inputs_embeds = inputs_embeds)
        except:
            value = self.v_proj(hidden_states)
            
        try:
            query, transport_plan = self.q_proj(hidden_states, layer_wise_forward = layer_wise_forward, active_layer = active_layer, active_adapter = active_adapter, weighted_matrix = weighted_matrix, inputs_embeds = inputs_embeds)
        except:
            query = self.q_proj(hidden_states)
            
        try:
            key, transport_plan = self.k_proj(hidden_states, layer_wise_forward = layer_wise_forward, active_layer = active_layer, active_adapter = active_adapter, weighted_matrix = weighted_matrix, inputs_embeds = inputs_embeds)
        except:
            key = self.k_proj(hidden_states)
        ...
        # return outputs, present, transport_plan  # a, present, (attentions)
        
        return attn_output, attn_weights, present, transport_plan
...
class GPTNeoBlock(nn.Module):
    ...
    def forward(
        ...
        hidden_states, self_attn_weights, present_key_value, transport_plan = self.attn(
            hidden_states,
            layer_past=layer_past,
            attention_mask=attention_mask,
            head_mask=head_mask,
...
# class GPTNeoForCausalLM(GPTNeoModel):
    ...
    def forward(
        ...
        transport_plan_all = transformer_outputs[-1]
        ...
        if labels is not None:
            ...
            notreduced_loss = notreduced_loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))

            lm_logits = lm_logits.to(hidden_states.dtype)
            loss = loss.to(hidden_states.dtype)
            notreduced_loss = notreduced_loss.to(hidden_states.dtype)
