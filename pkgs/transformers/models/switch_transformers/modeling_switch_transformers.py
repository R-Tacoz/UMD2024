...
class OTRouter(SwitchTransformersTop1Router):

    def __init__(self, config: SwitchTransformersConfig):
        super().__init__(config)

        self.num_experts = config.num_experts
        expert_emb = torch.ones((config.hidden_size, config.num_experts))
        self.expert_emb = torch.nn.Parameter(expert_emb.cuda().requires_grad_())
        nn.init.xavier_uniform_(self.expert_emb)
    ...
    def _compute_router_probabilities(self, hidden_states: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # print('OTRouter hidden_states.shape:', hidden_states.shape)
        # router_logits = self.classifier(hidden_states)
        # print('router_logits.shape:', router_logits.shape)

        router_loss = torch.matmul(hidden_states, self.expert_emb)
        router_loss = nn.Softmax(dim = 2)(router_loss)
        router_logits = torch.ones_like(router_loss)

        # print('router_loss.shape:', router_loss.shape)
        # print(router_loss)

        token_marginal = (self.num_experts / router_loss.shape[1]) * np.ones(router_loss.shape[1])
        expert_marginal = np.ones(self.num_experts)
        for i in range(router_loss.shape[0]):
            obj_weights, _ = ipot_WD(token_marginal, expert_marginal, router_loss[i].detach().cpu().numpy(), beta=0.01, max_iter=200, L=1)
            router_logits[i] = torch.from_numpy(obj_weights).float().to(self.expert_emb.device)

        # print('router_logits.shape:', router_logits.shape)
        # Apply Softmax and cast back to the original `dtype`
        router_probabilities = nn.functional.softmax(router_logits, dim=-1, dtype=self.dtype).to(self.input_dtype)
        # print('router_probabilities.shape:', router_probabilities.shape)
        return router_probabilities, router_logits
